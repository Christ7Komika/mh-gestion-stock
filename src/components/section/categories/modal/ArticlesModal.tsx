...
import { getStoresByCategory } from "../../../../redux/features/stores";
...
const ArticlesModal = ({ setAction }: Props) => {
  ...
  const currentId = useSelector((state: RootState) => state.category.currentId);
  const articles = useSelector((state: RootState) => state.store.CategoryStore);
  const count = useSelector((state: RootState) => state.store.count);
  const isLoad = useSelector(
    (state: RootState) => state.store.isLoadByCategory
  );
  const isLoadCategory = useSelector(
    (state: RootState) => state.category.currentId
  );

  useEffect(() => {
    if (currentId) {
      getStoresByCategory(currentId, step, skip, "empty")(dispatch);
    }
  }, [currentId]);
  ...
  useEffect(() => {
    ...
    if (currentId) {
      ...
      getStoresByCategory(currentId, step, skip, searchValue)(dispatch);
    }
  }, [skip, count]);
  ...
  useEffect(() => {
    if (isValid && search) {
      if (currentId) {
        getStoresByCategory(currentId, 9, 0, search)(dispatch);
      }
    }

    if (isValid && !search) {
      if (currentId) {
        getStoresByCategory(currentId, 9, 0, "empty")(dispatch);
      }
    }
  }, [search, isValid]);
