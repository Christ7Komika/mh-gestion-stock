...
import { getStoresByWarehouse } from "../../../../redux/features/stores";
...
const ArticlesModal = ({ setAction }: Props) => {
  ...
  const currentId = useSelector(
    (state: RootState) => state.warehouse.currentId
  );
  const isLoadWarehouse = useSelector(
    (state: RootState) => state.warehouse.currentId
  );
  const articles = useSelector(
    (state: RootState) => state.store.WarehouseStore
  );
  const count = useSelector((state: RootState) => state.store.count);
  const isLoad = useSelector(
    (state: RootState) => state.store.isLoadByWarehouse
  );

  useEffect(() => {
    if (currentId) {
      getStoresByWarehouse(currentId, step, skip, "empty")(dispatch);
    }
  }, [currentId]);
  ...
  useEffect(() => {
    ...
    if (currentId) {
      ...
      getStoresByWarehouse(currentId, step, skip, searchValue)(dispatch);
    }
  }, [skip, count]);
  ...
  useEffect(() => {
    if (isValid && search) {
      if (currentId) {
        getStoresByWarehouse(currentId, 9, 0, search)(dispatch);
      }
    }

    if (isValid && !search) {
      if (currentId) {
        getStoresByWarehouse(currentId, 9, 0, "empty")(dispatch);
      }
    }
  }, [search, isValid]);
